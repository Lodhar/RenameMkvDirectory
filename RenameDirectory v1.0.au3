...
#pragma compile(Icon,"E:\_workspace\Ico\maker-movie-fenetres-icone-9140.ico")
...
;~    ConsoleWrite($filePath & @CRLF)
   Local $aArray = _FileListToArrayRec($filePath, $include & "|" & $exclude & "|" & $Exclude_Folders, $FLTAR_FILES , $FLTAR_RECUR , $FLTAR_SORT, $FLTAR_FULLPATH  ) ; Get MKV files
...
;~ 		 ConsoleWrite("Result: " & $result & @CRLF)
	  $iSize = Round(DirGetSize($filePath & "\" & $vElement) / 1024 / 1024 / 1024,2) & "G"
;~ 	  ConsoleWrite("Size of: " & $filePath & "\" & $vElement & " " & Round($iSize / 1024 / 1024 / 1024,2) & "G" & @CRLF)
;~ 	  ConsoleWrite("Size: " & Round($iSize / 1024 / 1024 / 1024,2) & "G" & @CRLF )
		 _ArrayAdd($resultArray,$result & " (" & $iSize & ")")
...
;~     Local $hGUI = GUICreate("Example",700,500,Default,Default,BitOR( $WS_SIZEBOX,$WS_SYSMENU ))
   Local $hGUI = GUICreate("Rename My Movies",800,500,Default,Default,Default)
...
;~ 	  For $vElement In $aArray
   ...
   Local $idListview = GUICtrlCreateListView("", 10, 10,750,400) ;,$LVS_SORTDESCENDING)
   ...
   _GUICtrlListView_AddColumn($idListview, "To rename", 350)
